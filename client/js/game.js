        ...
        var Game = Class.extend({
            ...
            loadMap: function () {
                ...
                this.map.ready(function () {
                    log.info("Map loaded.");
                    let tilesetIndex = self.renderer.upscaledRendering ? 0 : self.renderer.scale - 1;
            ...
            /**
             * tick - 主循环函数，负责更新游戏逻辑、渲染帧以及控制动画循环。
             *
             * 此函数在每一帧被调用，执行以下操作：
             * 1. 更新当前时间戳。
             * 2. 如果游戏已启动，则更新光标逻辑、执行更新器逻辑并渲染当前帧。
             * 3. 如果动画未停止，则继续请求下一帧动画。
             */
            tick: function () {
                // 获取当前时间戳并存储到实例变量中
             ...
             * 启动游戏循环。
             *
             * 该函数用于初始化并启动游戏的主要循环。它会调用 `tick` 方法来开始执行游戏逻辑，
             * 并将 `hasNeverStarted` 标志设置为 `false`，表示游戏已经启动过。
             * 最后，通过日志记录游戏循环已成功启动的信息。
             */
            start: function () {
                // 调用 tick 方法以启动游戏逻辑循环
                this.tick();
            ...
            connect: function (started_callback) {
                ...
                this.client.onWelcome(function (id, name, x, y, hp) {
                     ...
                     * 注册玩家开始路径移动时的回调函数。
                     *
                     * @param {Function} callback - 回调函数，接收一个参数，表示玩家的移动路径。
                     *   - [path] - 移动路径的坐标数组，每个元素是一个包含 [x, y] 坐标的数组。
                     *
                     * 此函数处理玩家移动逻辑，包括：
                     * - 判断玩家是否正在移动到战利品位置，并更新相关状态。
                     * - 若玩家未处于攻击状态，则发送移动指令到客户端。
                     * - 更新选中目标的位置信息及可见性。
                     * - 在移动端或平板设备上，绘制并检查目标区域的脏矩形。
                     */
                    ...
                    /**
                     * 监听玩家的仇恨检查事件，并遍历所有怪物，判断是否满足攻击条件。
                     * 如果怪物具有攻击性、未处于攻击状态，且玩家在怪物的仇恨范围内，
                     * 则触发玩家对怪物的仇恨行为。
                     */
                    self.player.onCheckAggro(function () {
                        // 遍历所有怪物对象
                        self.forEachMob(function (mob) {
                            // 检查怪物是否具有攻击性、未处于攻击状态，且玩家在其仇恨范围内
                    ...
                    /**
                     * 监听玩家被怪物仇恨（Aggro）的事件处理函数。
                     * 当怪物对玩家产生仇恨时，执行以下逻辑：
                     * 1. 检查怪物是否正在等待攻击玩家，以及玩家是否已被该怪物攻击。
                     * 2. 如果条件满足，则记录日志、发送仇恨通知给客户端，并设置怪物等待攻击玩家的状态。
                     *
                     * @param {Object} mob - 触发仇恨事件的怪物对象，需包含 id、isWaitingToAttack、waitToAttack 等方法或属性。
                     */
                    self.player.onAggro(function (mob) {
                        // 检查怪物是否未处于等待攻击状态，且玩家未被该怪物攻击
                    ...
                    /**
                     * 玩家每一步执行的逻辑处理函数。
                     * 该函数在玩家每次移动时被调用，用于处理与玩家位置相关的各种事件和状态更新。
                     */
                    self.player.onStep(function () {
                        // 如果玩家还有下一步动作，则注册玩家的当前位置信息
                        if (self.player.hasNextStep()) {
                            self.registerEntityDualPosition(self.player);
                        }

                        // 检查玩家是否处于区域切换的边界 tile 上，如果是则将区域切换请求加入队列
                        if (self.isZoningTile(self.player.gridX, self.player.gridY)) {
                            self.enqueueZoningFrom(self.player.gridX, self.player.gridY);
                        }

                        // 遍历所有攻击者，根据其与目标的距离决定行为：靠近或面向目标
                        self.player.forEachAttacker(function (attacker) {
                            if (attacker.isAdjacent(attacker.target)) {
                                attacker.lookAtTarget(); // 攻击者已接近目标，面向目标
                            } else {
                                attacker.follow(self.player); // 攻击者未接近目标，继续跟随玩家
                            }
                        });
                        ...
                        if ((self.player.gridX <= 85 && self.player.gridY <= 179 && self.player.gridY > 178) ||
                            ...
                            self.tryUnlockingAchievement("INTO_THE_WILD"); // 解锁 "INTO_THE_WILD" 成就
                        }

                        if (self.player.gridX <= 85 && self.player.gridY <= 293 && self.player.gridY > 292) {
                            self.tryUnlockingAchievement("AT_WORLDS_END"); // 解锁 "AT_WORLDS_END" 成就
                        }

                        if (self.player.gridX <= 85 && self.player.gridY <= 100 && self.player.gridY > 99) {
                            self.tryUnlockingAchievement("NO_MANS_LAND"); // 解锁 "NO_MANS_LAND" 成就
                        }

                        if (self.player.gridX <= 85 && self.player.gridY <= 51 && self.player.gridY > 50) {
                            self.tryUnlockingAchievement("HOT_SPOT"); // 解锁 "HOT_SPOT" 成就
                        }

                        if (self.player.gridX <= 27 && self.player.gridY <= 123 && self.player.gridY > 112) {
                            self.tryUnlockingAchievement("TOMB_RAIDER"); // 解锁 "TOMB_RAIDER" 成就
                        }
                     ...
                     * 当玩家停止寻路时触发的回调函数。
                     *
                     * @param {number} x - 玩家停止寻路时的网格X坐标。
                     * @param {number} y - 玩家停止寻路时的网格Y坐标。
                     */
                    self.player.onStopPathing(function (x, y) {
                        ...
                        if (self.isItemAt(x, y)) {
                            ...
                            try {
                                ...
                                self.client.sendLoot(item); // 通知服务器该物品已被拾取
                    ...
                    /**
                     * 为玩家对象注册一个路径请求处理函数。
                     * 当玩家需要寻找从当前位置到目标位置(x, y)的路径时，此函数会被调用。
                     *
                     * @param {number} x - 目标位置的x坐标
                     * @param {number} y - 目标位置的y坐标
                     * @returns {Array|undefined} 返回找到的路径数组，如果未找到则返回undefined
                     */
                    self.player.onRequestPath(function (x, y) {
                        // 初始化忽略列表，始终忽略玩家自身
                        let ignored = [self.player];

                        // 如果玩家当前有目标，则将该目标也加入忽略列表
                        if (self.player.hasTarget()) {
                            ignored.push(self.player.target);
                        }

                        // 调用findPath方法计算路径，并返回结果
                     ...
                     * 2. 停止玩家闪烁效果。
                     * 3. 设置玩家死亡精灵并播放死亡动画。
                     * 4. 在动画结束后移除玩家实体并停止客户端功能。
                     * 5. 通知所有攻击者停止攻击并进入空闲状态。
                     * 6. 淡出当前背景音乐并播放死亡音效。
                     */
